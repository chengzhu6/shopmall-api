...
import com.castellan.dao.*;
import com.castellan.pojo.*;
import com.castellan.service.IOrderService;
import com.castellan.util.BigDecimalUtil;
import com.castellan.util.DateTimeUtil;
import com.castellan.util.FTPUtil;
import com.castellan.util.PropertiesUtil;
import com.castellan.vo.OrderItemVo;
import com.castellan.vo.OrderProductVo;
import com.castellan.vo.OrderVo;
import com.castellan.vo.ShippingVo;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse cancel(Integer userId,Long orderNo){
        ...
        if(order.getStatus() != Const.OrderStatus.NO_PAY.getStatus()){
            return ServerResponse.createByErrorMessage("已付款,无法取消订单");
        }
        Order updateOrder = new Order();
        updateOrder.setId(order.getId());
        updateOrder.setStatus(Const.OrderStatus.CANCELED.getStatus());
    ...
    public ServerResponse<OrderVo> getOrderDetail(Integer userId,Long orderNo){
        Order order = orderMapper.selectByUserIdOrderNo(userId,orderNo);
        if(order != null){
            List<OrderItem> orderItemList = orderItemMapper.selectListByOrderNoUserId(orderNo,userId);
            OrderVo orderVo = assembleOrderVo(order,orderItemList);
            return ServerResponse.createBySuccess(orderVo);
        }
    ...
    public ServerResponse<String> manageSendGoods(Long orderNo){
        ...
        if(order != null){
            if(order.getStatus() == Const.OrderStatus.PAID.getStatus()){
                order.setStatus(Const.OrderStatus.SHIPPED.getStatus());
    ...
    private List<OrderVo> assembleOrderVoList(List<Order> orderList,Integer userId){
        ...
        for(Order order : orderList){
            ...
            if(userId == null){
                ...
                orderItemList = orderItemMapper.selectListByOrderNoUserId(order.getOrderNo(),userId);
            }
            OrderVo orderVo = assembleOrderVo(order,orderItemList);
    ...
    private Order assembleOrder(Integer userId,Integer shippingId,BigDecimal payment){
        ...
        order.setStatus(Const.OrderStatus.NO_PAY.getStatus());
        order.setPostage(0);
    ...
    public ServerResponse pay(Integer userId, Long orderNo, String path){
        ...
        String subject = new StringBuilder().append("castellanmall订单扫码支付，订单号").append(outTradeNo).toString();
        ...
        List<OrderItem> orderItems = orderItemMapper.selectListByOrderNoUserId(orderNo,userId);
    ...
    public ServerResponse aliCallBack(Map<String,String> params){
        ...
        if(Const.AliPayCallBack.TRADE_SUCCESS.equals(tradeStatus)){
            ...
            order.setStatus(Const.OrderStatus.PAID.getStatus());
            orderMapper.updateByPrimaryKeySelective(order);
        }
